...
const ayarlar = require('../../c-guard.json');
const config = require('../../c-guvenli.json');
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "Ozi System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	

  let channel = client1.channels.cache.get(ayarlar.patliyozlannnlog)
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "Ozi System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
client2.on("roleDelete", async role => {
  ...
  let channel = client2.channels.cache.get(ayarlar.patliyozlannnlog)
...
client2.on("roleCreate", async role => {
  ...
  uyecik.roles.set([ayarlar.patlatmagirisirol]).catch(err => { })
...
client2.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  let channel = client3.channels.cache.get(ayarlar.patliyozlannnlog)
...
client3.on("channelDelete", async channel => {
  ...
  let channel2 = client3.channels.cache.get(ayarlar.patliyozlannnlog)
...
client3.on("webhookUpdate", async (channel) => {
  ...
  channel.guild.channels.cache.get(ayarlar.patliyozlannnlog).send(`${entry.executor} üyesi tarafından sunucuda izinsiz webhook açıldı, webhook silinip ve banlandı.\n─────────────────────\nYetkili: (${entry.executor} - \`${entry.executor.id}\`)\n─────────────────────\nTarih: \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\``)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
client1.on("ready", () => {
    client1.channels.cache.get("899273633283866753").join(); //SES KANALI İDSİ GİRİN!
  });
...
////
client2.on("ready", () => {
    client2.channels.cache.get("899273633283866753").join(); //SES KANALI İDSİ GİRİN!
  });
...
///
client3.on("ready", () => {
    client3.channels.cache.get("899273633283866753").join(); //SES KANALI İDSİ GİRİN!
  });
...
client1.on("roleDelete", role => {
  client1.channels.cache.get("899273632696631365").send("<a:kirmiziok:901441275381817426> Silinen Rol ID: `" + role.id + "`");
  client1.channels.cache.get("899273632696631365").send("<a:kirmiziok:901441275381817426> Silinen Rol İsim: `" + role.name + "`");
});

client1.on("channelDelete", channel => {
  client1.channels.cache.get("899273632696631365").send("<a:kirmiziok:901441275381817426> Silinen Kanal ID: `" + channel.id + "`");
  client1.channels.cache.get("899273632696631365").send("<a:kirmiziok:901441275381817426> Silinen Kanal İsim: `" + channel.name + "`");
});
